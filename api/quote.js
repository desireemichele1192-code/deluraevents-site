...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const html = `<div style="font-family:ui-sans-serif,system-ui,-apple-system,Segoe UI,Roboto">
      ...
      <table style="border-collapse:collapse">${fields.map(([k,v])=>`<tr><td style="padding:6px 12px;font-weight:600">${k}</td><td style="padding:6px 12px">${esc(v)}</td></tr>`).join('')}</table>
    </div>`;

    await resend.emails.send({ from, to, subject: `New ${body.arm||'Inquiry'} — ${body.name||'No Name'}`, html });

    const webhook = process.env.SHEETS_WEBHOOK_URL;
    if (webhook && typeof fetch === 'function') {
      try {
        await fetch(webhook, { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ createdAt:new Date().toISOString(), ...body }) });
